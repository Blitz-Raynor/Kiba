...
async def _(bot: Bot, event: Event, state: T_State):
    help_str = '''▼ 漂流社区功能 | Commands For Kiba Community                                          
------------------------------------------------------------------------------------------------------------------------------
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if mt == "guild":
        await plp_settings.finish("▿ Kiba Community | 漂流社区设置\n暂不支持频道的漂流瓶设置。")
        return
    try:
        if len(argv) == 3:
            success = 400
            ...
            if m['role'] != 'owner' and m['role'] != 'admin' and str(m['user_id']) not in Config.superuser:
                await plp_settings.finish("▿ Kiba Community | 漂流社区设置\n这个...只有管理员可以设置Kiba Community | 漂流社区。")
                return
    except Exception as e:
        await plp_settings.finish(f"▿ Kiba Community | 漂流社区设置 - 现在是私聊？\n群的瓶子开关在私聊是无法设置的，或您输入了错误的群号(犽不在这个群)。\n如果需要在私聊处理成员的拉黑，您需要在命令后面添加犽所在群号以便查验您是否为管理员。\n请在如果不是私聊，看下下面的错误记录。\n[Exception Occurred]\n{e}")
        return
    try:
        if argv[0] == "完全启用":
            if len(argv) == 1:
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if mt == "guild":
        ...
        data = await c.fetchone()
        if data is None:
            await plp_reply.send(f"▿ [Sender: {nickname}]\n  Kiba Community | 漂流社区 - 错误\n在频道内，您需要绑定 QQ 号才可使用Kiba Community | 漂流社区。请进行绑定后再试一次。")
            return
        else:
            user = data[0]
    try:
    ...
    try:
        ...
        if argv[0].find("CQ:image") != -1:
            ...
            await db.commit()
            await plp_insert.finish(f"▾ [Sender: {nickname}]\n  Kiba Community | 漂流社区: 扔瓶子 - 完成\n您的 文字 漂流瓶(ID: {plpid})已经扔出去啦!\n请注意: 如果您的瓶子包含了不应在漂流瓶内出现的内容，您可能会受到 Kiba Community 的部分功能封禁或相应处置。如果需要撤回瓶子，请使用 “删瓶子” 指令。")
            return
    except Exception as e:
        pass
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    try:
        if len(argv) > 1:
            ...
            data = await c.fetchone()
            if data is None:
                await plp_find.finish(f"▿ [Sender: {nickname}]\n  Kiba Community | 漂流社区: 捞瓶子 - 没有瓶子\n啊呀....小犽这目前一个瓶子都莫得。要不先扔一个看看？")
                return
            else:
                if data[4] == 0:
                    await plp_find.send(f"▾ [Sender: {nickname}]\n  Kiba Community | 漂流社区: 瓶子\nID: {data[0]} | {data[2]}({data[1]})\n👓 {data[5] + 1} | 💬 {data[6]}\n{data[3]}")
            ...
            if len(data) == 0:
                ...
                if data is None:
                    ...
                    if data[4] == 0:
                        msg1 = f"▾ [Sender: {nickname}]\n  Kiba Community | 漂流社区: 瓶子 - 定向 ID 查找: {argv[0]}\n{data[2]}({data[1]})\n👓 {data[5] + 1} | 💬 {data[6]}\n{data[3]}"
                ...
                msg = f"▾ [Sender: {nickname}]\n  Kiba Community | 漂流社区: 瓶子 - 定向 QQ 查找: {data[0][2]}({argv[0]})"
                if len(data) > 5:
        ...
        try:
            ...
            if ids.startswith("group"):
                ...
                if m['role'] != 'owner' and m['role'] != 'admin' and str(m['user_id']) not in Config.superuser:
                    ...
                    data = await c.fetchone()
                    if data is None:
                        await delete_plp.finish("▿ Kiba Community | 漂流社区\n找不到这个瓶子捏，看看您的 漂流瓶 ID 是否输入正确？")
                        return
                    else:
                        if event.user_id == data[1]:
                            ...
                            await db.commit()
                            await delete_plp.finish(f"▾ Kiba Community | 漂流社区 - 删除完成\n已删除 ID 是 {argv[0]} 的漂流瓶。")
                            return
                        else:
