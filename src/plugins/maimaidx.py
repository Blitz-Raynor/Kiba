    ...
    if len(result_set) > 50:
        try:
            ...
            if ids.startswith("group"):
                await inner_level.finish(f"▿ 搜索结果过多\n结果太多啦！阿犽找到了{len(result_set)} 条符合范围的歌曲，请您先缩小搜索范围或在私聊中来显示所有结果。若私聊阿犽没有反应，请先加阿犽为好友再试。")
                return
            else:
                s += "\n警告: 搜索结果过多，有可能显示不全。"
                resultnum = 0
    ...
    await jrwmnew.send(Message([
            {
                "type": "image",
                "data": {
                    "file": f"base64://{str(image_to_base64(pic), encoding='utf-8')}"
                }
            },
            {
                "type": "text",
                "data": {
                    "text": f"输入命令 '/text 今日运势' 来获取文字版运势。"
                }
            }
        ]))
        ...
        if len(song_remain_difficult) > 0:
            ...
            if not res.groups()[2]:
                ...
                if data is None:
                    ...
                    await c.execute(f'update plate_table set platenum={platenum} where id={qq}')
                await db.commit()
                msg += '\n您在 Kiba 上自主查询 B40 或 B50 时，此铭牌将展现在您的姓名框处。'
...
@baselist.handle()
async def _(bot: Bot, event: Event, state: T_State):
    ds = str(event.get_message()).strip()
    nickname = event.sender.nickname
